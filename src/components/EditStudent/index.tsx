...
export default function EditStudent({
  ...
  const [role_id, setRoleId] = useState<number>(1)
  ...
  async function handleEditStudent(e: React.FormEvent) {
    ...
    try {
      const res = await editUser(username, studentToSend)
      console.log('Učenik izmijenjen:', res)
      handleAlert()
    } catch (error) {
      console.error('Greška prilikom izmjene učenika:', error)
    }
  }
    ...
    >
      ...
      <FormControl fullWidth disabled>
        <Select id="role" value={role_id}>
          <MenuItem value={1}>Učenik</MenuItem>
        </Select>
      </FormControl>
      <TextField
        label="Unesite JMBG.."
