...
import { createStudent, editUser } from '@/utils/apiService'
import { StudentProps } from '../../../types'

export default function EditLibrarian({ data, nameInputRef }: StudentProps) {
  ...
  const [jmbg, setJmbg] = useState(data?.jmbg ? data?.jmbg : '')
  ...
  async function handleEditLibrarian(e: React.FormEvent) {
    e.preventDefault()
    setSubmitted(true)
    ...
    const librarianToSend = {
      ...
      role_id,
      password,
      // profile_picture nije uključen jer ne može ići kao base64 ili fajl bez FormData
    }

    try {
      const res = await editUser(username, librarianToSend)
      console.log('BIbliotekar izmijenjen:', res)
      handleAlert()
    } catch (error) {
      console.error('Greška prilikom izmjene bibliotekara', error)
    }
  }
    ...
    >
      ...
      {showAlert && (
        <Alert
          ...
          variant="filled"
        >
          Bibliotekar uspjesno izmijenjen!
        </Alert>
      )}
    </Box>
  )
